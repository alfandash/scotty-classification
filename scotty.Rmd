...
For models to predict, we only have choice choose model that accept only factor, or categorical predictor variable and categorical target variable, the model we can 
...
```{r}
confusionMatrix(logistict_prediction, train_validation$coverage, positive = "sufficient")
```


```{r}
dtree_model <- ctree(formula = coverage ~ src_area + day + hour, data = train_training)
```
...
```{r}
confusionMatrix(dtree_prediction, train_validation$coverage)
```
